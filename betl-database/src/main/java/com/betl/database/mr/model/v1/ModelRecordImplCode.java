...
import com.betl.database.util.JavaStringObject;
...
public class ModelRecordImplCode {
	...
	public String gengerate() {
		...
		String fieldStr = conf.get(BasicConstants.DATABASE_TABLE_COLUMNS);
		logger.debug("gengerate,fieldStr={}", fieldStr);
		String[] cols =cf.databaseColumns();
		 ...
		 */
		code.append("package com.betl.database.mr.model.v1;" + newLine);
		...
		code.append("res");
		for (String col : cols) {
			code.append(".append(this."+col+").append(\""+conf.get(BasicConstants.HDFS_COLUMNS_SPLIT)+"\")" + newLine);
		}
		code.append(";" + newLine);
	...
	public String compile(String code) throws ClassNotFoundException, MalformedURLException{
		logger.debug("compile,code={}",code);
	    ...
	    logger.debug("compile,codeCompileFlag={}",success);
